package com.plavaga.document.duke.mongodb;
...
import com.plavaga.document.duke.ConfigWriter;
...
import com.plavaga.document.duke.datasources.ColumnarDataSource;
import com.plavaga.document.duke.datasources.RecordBuilder;
...
public class MongoDBDataSource extends ColumnarDataSource {
	...
	private boolean			noTimeOut		= false;		// by default we
															// don't set that
															// flag
	...
	private String			query			= "{}";			// default: all
															// documents
	...
	// ----------
	// Setters: bean properties (Note: are "-" separated, instead of cammelCase
	// formatted)
	// ----------
	...
	public void setPortNumber(int parsedPort) {
	...
	public void setDbAuth(String authdb) {
		...
		/*
		 * I hate your 2 space identation, Lars... makes the code a little less
		 * unreadable
		 */
	...
	public RecordIterator getRecords() {
		...
		try {
			...
			// authentication mecanism via MONGODB-CR authentication
			// http://docs.mongodb.org/manual/core/authentication/#authentication-mongodb-cr
			if (auth.equals(AUTH_ON_DB)) {
				verifyProperty(username, "user-name");
				verifyProperty(password, "password");

				mongo = new MongoClient(new ServerAddress(mongouri, port), Arrays.asList(MongoCredential.createMongoCRCredential(username, dbname, password.toCharArray())));
			}
			else if (auth.equals(AUTH_ON_ADMIN)) {
				verifyProperty(username, "user-name");
				verifyProperty(password, "password");

				mongo = new MongoClient(new ServerAddress(mongouri, port), Arrays.asList(MongoCredential.createMongoCRCredential(username, AUTH_ON_ADMIN, password.toCharArray())));
			}
			else {
			...
			// See:
			// http://api.mongodb.org/java/current/com/mongodb/DBCursor.html#addOption(int)
			// and
	...
	public class MongoDBIterator extends RecordIterator {
		...
		public Record next() {
			...
			try {
				...
				for (Column col : getColumns()) {
					// TODO: identify arrays (containing values or DBObjects) in
		...
		// When the DOT is actually part of the field name, it does not work
		// properly
		// If this is your case, one day you will have to update it:
