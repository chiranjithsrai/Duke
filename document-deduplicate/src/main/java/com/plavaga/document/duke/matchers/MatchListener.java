...
/**
 * Interface implemented by code which can receive notifications that two
 * records are considered to match.
 *
 * <p>
 * Note that when running Duke with multiple threads, the matches(),
 * matchesPerhaps(), and noMatchFor() methods need to be thread-safe.
 */
public interface MatchListener {
	...
	/**
	 * Notification that the two records match. There will have been a previous
	 * startRecord(r1) notification.
	 */
	public Set<Object> matches(Record r1, Record r2, double confidence);

	/**
	 * Notification that the two records might match. There will have been a
