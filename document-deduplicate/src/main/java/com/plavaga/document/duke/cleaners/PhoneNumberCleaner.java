...
/**
 * <p>
 * Cleaner for international phone numbers. It assumes that it can get the same
 * phone number in forms like:
 *
 * <ul>
 * <li>0047 55301400
 * <li>+47 55301400
...
public class PhoneNumberCleaner implements Cleaner {
	...
	public String clean(String value) {
		...
		if (ccode == null) {
			...
			if (value.startsWith(ccode.getPrefix())) {
				value = value.substring(ccode.getPrefix().length()); // strip
																		// off
			}
			// ccode
	...
	private static Map<String, CountryCode> initcodes() {
		...
		ccs.put("91", new CountryCode("91", true, 10)); // IN
		ccs.put("0", new CountryCode("91", true, 10)); // IN
		...
		ccs.put("971", new CountryCode("971", true, 7, 9)); // United Arab
															// Emirates
